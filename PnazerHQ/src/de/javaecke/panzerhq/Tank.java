...
public class Tank extends GameObject{
	...
	private boolean ableToShot = true;
	...
	public void setTurningVolocity(double turningSpeed){
	...
	public void setCannonColor(Color canonColor){
		this.cannonColor = canonColor;
	}
	...
	public Missile shoot(){
		...
		double tankCenterX = getObjectPosition().getX();
		double tankCenterY = getObjectPosition().getY();
	...
	private void paintTankStatusBar(Graphics2D g2d){
		
		double barOffsetY = getHeight() * 0.6;
		
		//paint Tank Energy Bar
		g2d.setColor(Color.DARK_GRAY);
		RoundRectangle2D tankEnergBarFrame = new RoundRectangle2D.Double(Math.round(getObjectPosition().getX()) - 1,
											Math.round(getObjectPosition().getY() - barOffsetY) -1,
											getWidth() + 1, 6, 0, 0);
		g2d.draw(tankEnergBarFrame);
		if(getEnergy() > 3){
		...
		RoundRectangle2D tankEnergyBar = new RoundRectangle2D.Double(Math.round(getObjectPosition().getX()), 
										...
										getWidth() / getLoadingTime() * energy, 5, 0, 0);
